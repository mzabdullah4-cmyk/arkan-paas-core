...
def process(item: Dict[str, Any]) -> None:
    ...
    try:
        ...
        job = requests.get(f"{CONTROL_PLANE_URL}/internal/jobs/{job_id}/payload", timeout=30)
        ...
        body = {"sql": payload["content"]} if engine == "sqlglot" else {"source": payload["content"]}
        result = requests.post(f"{engine_url}/analyze", json=body, timeout=300)
        result.raise_for_status()
        analysis = result.json()
        if analysis.get("status") == "error":
            raise ValueError(analysis.get("error", "Engine analysis failed"))
        callback(job_id, "complete", {"result": {
            "job_id": job_id,
        ...
        try:
            ...
            # Leave the item available for the next retry if the control plane is unavailable.
            raise
