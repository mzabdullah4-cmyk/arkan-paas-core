...
app = FastAPI(title="Arkan Control Plane", version="0.3.0")
...
@app.get("/")
def root() -> dict:
    return {"status": "online", "service": "Arkan API Gateway", "version": "0.3.0"}
...
def upload_artifact(project_id: str, language: str, file: UploadFile = File(...)) -> dict:
    if not fetch_project(project_id):
        raise HTTPException(404, "Project not found")
    raw = await file.read()
    if not raw:
...
def create_job_endpoint(payload: JobCreate) -> dict:
    ...
    try:
        ...
        raise HTTPException(400, f"Unsupported engine: {payload.engine}")
