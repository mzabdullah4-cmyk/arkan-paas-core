...
def create_artifact(
    ...
    with _connect() as conn:
        conn.execute(
            "INSERT INTO artifacts (id, project_id, name, language, content, storage_path, sha256, size) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (artifact_id, project_id, name, language, content, storage_path, sha256, size),
        )
...
def create_job(project_id: str, artifact_id: str, engine: str, max_attempts: int = 3) -> Dict[str, Any]:
    ...
    with _connect() as conn:
        conn.execute(
            "INSERT INTO jobs (id, project_id, artifact_id, engine, max_attempts) VALUES (?, ?, ?, ?, ?)",
            (job_id, project_id, artifact_id, engine, max_attempts),
        )
...
def job_payload(job_id: str) -> Optional[Dict[str, Any]]:
    with _connect() as conn:
        row = conn.execute(
            "SELECT j.id, j.project_id, j.artifact_id, j.engine, j.status, j.attempts, j.max_attempts, a.name, a.language, a.content, a.storage_path FROM jobs j JOIN artifacts a ON a.id = j.artifact_id WHERE j.id = ?",
            (job_id,),
        ).fetchone()
...
def update_job(
    ...
    with _connect() as conn:
        if increment_attempt:
            conn.execute(
                "UPDATE jobs SET attempts = attempts + 1, status = ?, error = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                (status, error, job_id),
            )
        else:
            conn.execute(
