...
def init_db() -> None:
    with _connect() as conn:
        ...
        # Safe migration for databases created by the first scaffold.
...
def _all(rows: List[sqlite3.Row]) -> List[Dict[str, Any]]:
...
def list_projects() -> List[Dict[str, Any]]:
    with _connect() as conn:
        return _all(conn.execute("SELECT * FROM projects ORDER BY created_at DESC").fetchall())
...
def create_artifact(project_id: str, name: str, language: str, content: str) -> Dict[str, Any]:
    ...
    with _connect() as conn:
        conn.execute("INSERT INTO artifacts (id, project_id, name, language, content) VALUES (?, ?, ?, ?, ?)", (artifact_id, project_id, name, language, content))
...
def list_artifacts(project_id: str) -> List[Dict[str, Any]]:
    with _connect() as conn:
        return _all(conn.execute("SELECT * FROM artifacts WHERE project_id = ? ORDER BY created_at DESC", (project_id,)).fetchall())


def create_job(project_id: str, artifact_id: str, engine: str) -> Dict[str, Any]:
    job_id = str(uuid.uuid4())
    with _connect() as conn:
        conn.execute("INSERT INTO jobs (id, project_id, artifact_id, engine) VALUES (?, ?, ?, ?)", (job_id, project_id, artifact_id, engine))
...
def list_jobs() -> List[Dict[str, Any]]:
    with _connect() as conn:
        return _all(conn.execute("SELECT * FROM jobs ORDER BY created_at DESC").fetchall())


def job_payload(job_id: str) -> Optional[Dict[str, Any]]:
    with _connect() as conn:
        row = conn.execute(
            "SELECT j.id, j.project_id, j.artifact_id, j.engine, a.name, a.language, a.content "
            "FROM jobs j JOIN artifacts a ON a.id = j.artifact_id WHERE j.id = ?",
            (job_id,),
        ).fetchone()
